...
Input file:
- data/iflix.txt            : list domain (1 per baris)
- data/ssh_accounts.json    : (opsional) hanya untuk menyimpan daftar akun (add/edit/delete). Tidak dipakai untuk request.
"""
...
IFLIX_FILE = str(ROOT_DIR / "data" / "iflix.txt")
SSH_FILE = str(ROOT_DIR / "data" / "ssh_accounts.json")
...
def _run_multihost_report_worker(selected_host: str, path: str, ip: str, ua: str, ssh_host: str) -> None:
    ...
    # save hit payloads
    if hit_payloads:
        outpath = str(ROOT_DIR / "output" / "hit_payloads.txt")
        try:
...
def run_multi_domain_mode(domains: List[str]) -> None:
    ...
    Saves matching domains to output/multi_200.txt.
    """
    clear_screen()
    ...
    # save matches
    if matches:
        out = str(ROOT_DIR / "output" / "multi_200.txt")
        try:
...
def save_accounts(accounts: List[dict]) -> bool:
    try:
        Path(SSH_FILE).parent.mkdir(parents=True, exist_ok=True)
