...
def emergency_repair():
    """Berjalan jika folder 'app' atau modul update hilang/rusak."""
    print("\n" + "!" * 50)
    print("🚨 ERROR: SISTEM RUSAK ATAU FILE HILANG!")
    print("Mencoba melakukan pemulihan otomatis dari GitHub...")
    print("!" * 50 + "\n")

    # Pastikan ini dijalankan di dalam folder repo git
    if not os.path.isdir(".git"):
        print("❌ Folder ini bukan repository git (.git tidak ditemukan).")
        print("Saran: Jalankan dari folder repo atau lakukan 'git clone' ulang.")
        sys.exit(1)
...
# Mencoba memanggil fungsi update dari file terpisah (service/git.py)
try:
